package uet.invincible.assistant.algorithm;
...
public class Calculator {
    public static int fromStringToExpression(String s) {
        ...
        final int NHAN = 3;
        final int CHIA = 4;
        final String[] oneDigitNumber = {"không", "một", "hai", "ba", "bốn", "năm", "sáu", "bảy", "tám", "chín", "mười"};
            ...
            } else
                {
                    for (int j = 0; j < oneDigitNumber.length; j++) {
                        if (words[i].equals(oneDigitNumber[j])) {
                        	Log.d("words[i]", words[i] + " " + oneDigitNumber[i]);
                            numbers.add(j);
                            continue;
                        }
                    }
