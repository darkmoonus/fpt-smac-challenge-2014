...
import uet.invincible.assistant.fragments.CanvasFragment;
...
public class RobotMainActivity extends BaseActivity implements OnClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mFragmentManager.beginTransaction().add(R.id.robot_canvas, CanvasFragment.getInstance(RobotMainActivity.this)).commit();
	}
	@SuppressLint("NewApi")
	...
	public void speak(String message, String language) {
		try {
			RobotTextToSpeech.say(getConnectedRobot(), message, language);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					RobotGesture.runGesture(getConnectedRobot(), name);
				} catch (RobotException e) {
					e.printStackTrace();
				}
			}
